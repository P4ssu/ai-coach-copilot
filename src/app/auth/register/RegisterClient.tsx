...
export function RegisterClient({ token, email: emailParam }: RegisterClientProps) {
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    try {
      ...
      // Accept invite and link to coach
      const acceptResult = await acceptInviteAndCreateClient(
        inviteToken,
        inviteEmail,
        authData.user.id,
        formData.name
      ...
      if (!acceptResult.success) {
        setError(acceptResult.error || 'Failed to link to coach')
        setRegistering(false)
        return
      }
        ...
        <div className="max-w-md w-full space-y-6 text-center">
          ...
          <div className="space-y-3">
            <p className="text-sm text-zinc-400">
              If you believe this is a mistake, contact your coach or request a new invite link.
            </p>
            <Link
              href="/"
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-4">
      <div className="max-w-md w-full space-y-6">
        {/* Header */}
        <div className="text-center">
          <h1 className="text-3xl font-bold mb-2">Join Vexlon</h1>
          <p className="text-zinc-400">Complete your registration</p>
        </div>
